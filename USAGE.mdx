...
#### 3. Newsletter Campaign with Analytics

Send a newsletter to segmented audience:

```bash
# Send newsletter to premium subscribers
ahasend messages send \
  --from newsletter@media.com \
  --recipients premium-subscribers.csv \
  --subject "Weekly Digest - {{week_of}}" \
  --html-template newsletter-premium.html \
  --text-template newsletter-premium.txt \
  --global-substitutions newsletter-data.json \
  --track-opens true \
  --track-clicks true \
  --tags newsletter,premium,weekly \
  --max-concurrency 3 \
  --progress \
  --show-metrics \
  --output json > campaign-results.json
