...
- `on_reception` - Email received by AhaSend
- `on_delivered` - Email delivered to recipient
- `on_transient_error` - Temporary delivery failure
- `on_failed` - Permanent delivery failure
- `on_bounced` - Email bounced
- `on_suppressed` - Email suppressed
- `on_suppression_created` - New suppression added
- `on_dns_error` - DNS resolution failure
- `on_opened` - Email opened by recipient
- `on_clicked` - Link clicked in email
...
ahasend webhooks trigger my-webhook-id --events "on_delivered"

# Simulate a complete email lifecycle
ahasend webhooks trigger my-webhook-id \
  --events "on_reception,on_delivered,on_opened,on_clicked"

# Test error handling
ahasend webhooks trigger my-webhook-id \
  --events "on_transient_error,on_failed,on_bounced"
...
Routes allow you to set up email forwarding and processing rules.
...
ahasend webhooks trigger webhook-id \
  --events "on_reception,on_bounced"

# Test suppression flow
ahasend webhooks trigger webhook-id \
  --events "on_suppressed,on_suppression_created"

# Test error scenarios
ahasend webhooks trigger webhook-id \
  --events "on_transient_error,on_failed,on_dns_error"
```
...
- Visit the [AhaSend Documentation](https://docs.ahasend.com)
- Check the [API Reference](https://docs.ahasend.com/api)
